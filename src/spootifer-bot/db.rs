...
pub(crate) fn get_user_guilds_by_guild_id(
    ...
    let r = q?
        .query_map([guild_id], |row: &Row| -> rusqlite::Result<UserGuild> {
            Ok(UserGuild {
                user_id: row.get(0)?,
...
pub(crate) fn update_user_guild_spotify_playlist_id(
    conn: &Arc<Mutex<Connection>>,
    ...
    let mut q = c.prepare(
        "UPDATE user_guilds SET spotify_playlist_id = ? WHERE discord_guild_id = ? AND user_id = ?",
    )?;
...
pub(crate) fn get_user_guild_by_user_id_and_guild_id(
    conn: &Arc<Mutex<Connection>>,
    guild_id: String,
    user_id: i64,
) -> Result<UserGuild> {
...
pub(crate) fn get_oauth_token_by_user_id(
    conn: &Arc<Mutex<Connection>>,
    user_id: i64,
) -> Result<OAuthToken> {
    let c = match conn.try_lock() {
    ...
    let q = c.prepare("SELECT user_id, refresh_token, access_token, expiry_time, token_type, deleted_at, created_at, updated_at, for_service FROM oauth_tokens WHERE user_id = ?;");

    let r = q?.query_row([user_id], |r| -> rusqlite::Result<OAuthToken> {
        Ok(OAuthToken {
            user_id: r.get(0)?,
...
pub(crate) fn insert_oauth_token(conn: &Transaction, token: OAuthToken) -> Result<i64> {
    let q = conn.prepare("INSERT INTO oauth_tokens (user_id, refresh_token, access_token, expiry_time, token_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
